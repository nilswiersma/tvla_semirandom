...
def sub_bytes(s):
    if DUMP_STATE: print(f'{"sub_bytes":20s}: {matrix2bytes(s).hex()}')
    for i in range(4):
        for j in range(4):
            s[i][j] = s_box[s[i][j]]
...
def inv_sub_bytes(s):
    if DUMP_STATE: print(f'{"inv_sub_bytes":20s}: {matrix2bytes(s).hex()}')
    for i in range(4):
        for j in range(4):
...
def shift_rows(s):
    if DUMP_STATE: print(f'{"shift_rows":20s}: {matrix2bytes(s).hex()}')
...
def inv_shift_rows(s):
    if DUMP_STATE: print(f'{"inv_shift_rows":20s}: {matrix2bytes(s).hex()}')
    s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
    s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
    s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]

def add_round_key(s, k):
    if DUMP_STATE: print(f'{"add_round_key":20s}: {matrix2bytes(s).hex()}')
    for i in range(4):
        for j in range(4):
            s[i][j] ^= k[i][j]
...
def mix_columns(s):
    if DUMP_STATE: print(f'{"mix_columns":20s}: {matrix2bytes(s).hex()}')
    for i in range(4):
        mix_single_column(s[i])


def inv_mix_columns(s):
    if DUMP_STATE: print(f'{"inv_mix_columns":20s}: {matrix2bytes(s).hex()}')
